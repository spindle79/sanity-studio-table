...
import { getByPath, isParseableDate } from "./utils";
...
export interface TableProps<TableData, AdditionalRowTableData> {
  ...
  loading?: boolean;
  /**
   * Dot-separated path to the unique identifier of the row.
   * Example: `"_id"` for top-level fields, `"document._id"` for nested.
   */
  rowId: string;
  rowActions?: ({
    datum,
  ...
  /**
   * Element to use as the virtualizer's scroll parent. Pass the ref to
   * whatever container in your studio plugin layout owns the scroll axis.
   */
  scrollContainerRef: HTMLDivElement | null;
  /**
   * If true, the table renders edge-to-edge. By default, the table is
   * inset to align with Sanity Studio's standard container width via
   * the studio theme's `v2.container[3]` token.
   */
...
const TableInner = <TableData, AdditionalRowTableData>({
  ...
  const filteredData = useMemo(() => {
    const filteredResult =
      searchTerm && searchFilter ? searchFilter(data, searchTerm) : data;
      ...
      const [aValue, bValue]: (number | string)[] = [a, b].map(
        (sortValue) =>
          (sortColumn?.sortTransform?.(
            ...
            sort.direction
          ) ?? getByPath(sortValue, sort.column)) as number | string
      );

      if (
        typeof aValue === "string" &&
        typeof bValue === "string" &&
        !isParseableDate(aValue) &&
        !isParseableDate(bValue)
      ) {
  ...
  const renderRow = useMemo(
    () =>
      function TableRow(
        ...
        const cardKey = loading
          ? `skeleton-${datum.index}`
          : String(getByPath(datum, rowId));
        ...
        return (
          <Card
            ...
            {...cardRowProps}
          >
            {amalgamatedColumnDefs.map(
              ({ cell: Cell, style, width, id, sorting = false }) => (
                <Fragment key={String(id)}>
                  <Cell
                    datum={
                      { ...datum, isLoading: loading } as RowDatum<
                        TableData,
                        AdditionalRowTableData
                      >
                    }
                    cellProps={{
                      as: "td",
  ...
  const headers = useMemo(
    () =>
      amalgamatedColumnDefs.map(({ cell: _cell, sortTransform: _st, ...header }) => ({
        ...header,
        id: String(header.id),
      })),
  ...
  // The Sanity v2 theme exposes a `container` token array via `theme.sanity.v2`.
  // Older studio themes don't have it; fall back to 0 (no inline padding cap).
  const maxInlineSize =
    (!hideTableInlinePadding &&
      (theme.sanity as unknown as { v2?: { container?: number[] } })?.v2
        ?.container?.[3]) ||
    0;
  ...
  const tableContent = () => {
    if (loading) return renderLoadingRows(renderRow);
    if (filteredData.length === 0) return emptyContent;
  ...
  const themeSpace =
    (theme.sanity as unknown as { v2?: { space?: number[] } })?.v2?.space?.[3] ?? 0;
...
/**
 * Virtualized table primitive for Sanity Studio plugins.
 *
 * Wraps `TableInner` in a `TableProvider` so sort + search state lives
 * inside the table by default. Mount your own `<TableProvider>` higher
 * up in the tree if you need to share that state with components outside
 * the table.
 */
